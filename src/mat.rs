...
macro_rules! mat_index {
    ($V:ident, $N:expr, $M:expr) => {
        impl<T> Index<(usize, usize)> for $V<T>
        where
            T: Copy,
        {
            type Output = T;
        ...
        impl<T> IndexMut<(usize, usize)> for $V<T>
        where
            T: Copy,
        {
...
macro_rules! mat_op_mat {
    ($V:ident, $N:expr, $M:expr, $Trait:ident, $method:ident, $op:ident) => {
        impl<T> $Trait for $V<T>
        where
            T: Copy + Num + $Trait<Output = T>,
        {
            type Output = $V<T>;
...
macro_rules! mat_op_mat_assign {
    ($V:ident, $N:expr, $M:expr, $Trait:ident, $BaseTrait:ident, $method:ident, $op:ident) => {
        impl<T> $Trait<$V<T>> for $V<T>
        where
            T: Copy + Num + $BaseTrait<Output = T>,
        {
